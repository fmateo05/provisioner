...
$testurl = file_get_contents('/var/www/html/provisioner/api/account/' . $account . '/account.json');
...
$lsddir = '/var/www/html/provisioner/api/account/' . $account . '/' ;
  if(!file_exists($lsddir)) {
      shell_exec('mkdir -p ' . $lsddir);
	if(!file_exists($lsddir . 'account.json')){

           file_put_contents($lsddir .'account.json', $initial_data);

	}
}

//$devices = shell_exec($cmd_json_get_devices );
$output = file_get_contents($lsddir .'account.json') ;
...
$lsddir = '/var/www/html/provisioner/api/account/' . $account . '/' ;
  if(!file_exists($lsddir)) {
      shell_exec('mkdir -p ' . $lsddir);
	if(!file_exists($lsddir . 'accounts.json')){

           file_put_contents($lsddir .'accounts.json', $initial_data);

	}
}

file_put_contents('/var/www/html/provisioner/api/account/' . $account . '/account.json',  $postdoc) ;
...
file_put_contents('/var/www/html/provisioner/api/account/' . $account . '/account.json',  $putdoc) ;

...
header("Location: https://" . trim($query_domain_name)   .":444/" . $out['http_auth_username'] . "/" . $out['http_auth_password'] . '/' . $subpaths['3'] . '/'  . $subpaths['4']);
exit();
//header("Location: " . $query_domain_name   ."/" . $out['http_auth_username'] . "/" . $out['http_auth_password'] . '/' , true, 302);
//$result = json_encode($json,JSON_PRETTY_PRINT);
